...
All references are **soft** (no foreign key enforcement). Deleting a Character or Chapter:
- Removes its references from `TimelineEvent.relatedCharacters` / `relatedChapters`
