<script lang="ts">
  ...
  let formTouched = $state(false);         // user has typed — don't close on Escape
  let identityLoaded = $state(false);      // prevent reload on re-open
  let remoteUrlAutoGenerated = $state(false); // prevent re-fill after user clears

  // Load identity once when dialog opens — skip if already loaded
  $effect(() => {
    if (open && !identityLoaded) {
      loadIdentity();
    }
  });
  ...
  // Auto-fill remote URL from githubUser + projectName — only once per session
  $effect(() => {
    if (step === "remote" && githubUser && !remoteUrl && !remoteUrlAutoGenerated && projectName) {
...
{#if open}
  ...
  <div
    ...
    onkeydown={(e) => e.key === "Escape" && step === "identity" && !formTouched && skipRemote()}
  >
    <div
      class="modal-panel"
          ...
          <label class="modal-field">
            {t("git.remoteUrlLabel")}
              <input
                ...
                disabled={loading}
                oninput={() => { formTouched = true; remoteUrlAutoGenerated = true; }}
              />
          </label>
        {/if}
